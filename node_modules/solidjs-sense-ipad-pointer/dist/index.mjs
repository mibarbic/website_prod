import { createComponent, Portal, effect, template } from 'solid-js/web';
...
`;
const setStyles = (hideOriginCursor) => {
...
const contextMode = (cursor, props) => {
  const parallaxSpeed = {
  ...
  let isHovered = false;
  let cursorTarget;
  let cursorTargetStyle;
  const moveCursor = (e) => {
    if (!isHovered) {
      return gsapWithCSS.to(cursor, {
      ...
      if (isElHasProperty(cursorTarget, propNames.lift)) {
        ...
        gsapWithCSS.to(cursor, {
          duration: props.transitionSpeed,
          filter: "blur(8px)",
          x: cursorTarget.getBoundingClientRect().left + (e.clientX - cursorTarget.getBoundingClientRect().left - cursorTarget.clientWidth / 2) / parallaxSpeed.cursor,
          y: cursorTarget.getBoundingClientRect().top + (e.clientY - cursorTarget.getBoundingClientRect().top - cursorTarget.clientHeight / 2) / parallaxSpeed.cursor,
          backgroundImage: `radial-gradient(circle at ${e.clientX - cursorTarget.getBoundingClientRect().left}px ${e.clientY - cursorTarget.getBoundingClientRect().top}px, rgba(255,255,255,0.4), rgba(255,255,255,0))`
        });
      } else {
        gsapWithCSS.to(cursor, {
          duration: props.transitionSpeed,
          x: cursorTarget.getBoundingClientRect().left - (isElHasProperty(cursorTarget, propNames.noPadding) ? 0 : props.hoverPadding) + (isElHasProperty(cursorTarget, propNames.noParallax) ? 0 : (e.clientX - cursorTarget.getBoundingClientRect().left - cursorTarget.clientWidth / 2) / parallaxSpeed.cursor),
          y: cursorTarget.getBoundingClientRect().top - (isElHasProperty(cursorTarget, propNames.noPadding) ? 0 : props.hoverPadding) + (isElHasProperty(cursorTarget, propNames.noParallax) ? 0 : (e.clientY - cursorTarget.getBoundingClientRect().top - cursorTarget.clientHeight / 2) / parallaxSpeed.cursor),
          borderRadius: borderRadius * (isElHasProperty(cursorTarget, propNames.noPadding) ? 1 : 1.5),
          width: cursorTarget.clientWidth + (isElHasProperty(cursorTarget, propNames.noPadding) ? 0 : props.hoverPadding * 2),
          height: cursorTarget.clientHeight + (isElHasProperty(cursorTarget, propNames.noPadding) ? 0 : props.hoverPadding * 2)
        });
  ...
  const handleMouseOut = (e) => {
    ...
    if (cursorTarget) {
      let cTarget = cursorTarget;
      const cTargetStyle = cursorTargetStyle;
      gsapWithCSS.to(cTarget, {
        duration: props.transitionSpeed,
        x: 0,
        y: 0,
        scale: 1,
  ...
  const handleMouseLeave = (e) => {
    const target = e.target;
    if (!isElHasProperty(target) && !(target === document)) {
      return;
    }
    handleMouseOut(e);
  };
...
const _tmpl$ = /*#__PURE__*/template(`<div class="ipad-pointer"></div>`, 2);
const IpadPointer = props => {
  let refCursor;
  const clearCbs = []; // Default props
  ...
  onMount(() => {
    ...
    clearCbs.push( // Set base
    setStyles(props.hideOriginCursor), // Load mode when page is loaded
  ...
  return createComponent(Portal, {
    get children() {
      ...
      const _ref$ = refCursor;
      typeof _ref$ === "function" ? _ref$(_el$) : refCursor = _el$;

      _el$.style.setProperty("transform", "translate(-200px, -200px)");
      ...
      effect(_p$ => {
        const _v$ = `${defaultProps.radius}px`,
              _v$2 = `${defaultProps.radius}px`,
              _v$3 = defaultProps.hideCursorOutside ? 0 : 1;

        _v$ !== _p$._v$ && _el$.style.setProperty("height", _p$._v$ = _v$);
        _v$2 !== _p$._v$2 && _el$.style.setProperty("width", _p$._v$2 = _v$2);
        _v$3 !== _p$._v$3 && _el$.style.setProperty("opacity", _p$._v$3 = _v$3);
        return _p$;
      }, {
        _v$: undefined,
        _v$2: undefined,
